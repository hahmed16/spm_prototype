...
7. [Indicators & Risks](#7-indicators--risks)
...
Elements are connected through directed, weighted edges. An element can have multiple children and can participate in multiple strategies. Within a single strategy, each element has exactly one parent (or is a root node). Across strategies, the same element can be reused without duplication. The structure is a **Directed Weighted Graph**.
...
Additional fields are unlocked by the element type's declared capabilities (Budgetable, Trackable, VisionAligned).
...
### Strategy
A strategic plan that references a set of elements and defines how they are structured and connected in this context. A strategy owns its **edges** (the connections between elements) but not the elements themselves. A strategy has its own lifecycle status independent of its elements.
...
- **Uniqueness**: Each element can appear as a child of at most one parent within the same strategy — it either has one parent edge or is a root node, never both
...
### Capabilities
Each element type declares its capabilities. Capabilities unlock additional data fields, behaviors, and lifecycle rules.
...
- **Indicators**: Optionally, any element can have indicators linked to it within a strategy context. Indicators are not mandatory — they are linked only when relevant to the element
- **Risks**: Optionally, any element can have risks linked to it within a strategy context. Risks are not mandatory — they are linked only when the element faces identifiable risks
...
- **VisionAligned** is always independent and optional for any type
- Capabilities are defined once per type at org level and apply to all elements of that type
...
Each strategy defines which element types it uses and their level order:

- Level 1 (root): e.g., Perspective
- Level 2: e.g., Goal
- Level 3: e.g., Initiative

**This configuration is the authoritative build specification for the strategy.** It defines which element types belong at each level and which type is the expected parent of which type. Elements within the strategy should be linked following this configuration. The system validates links against the configuration and raises a warning when a link does not conform. Links outside the configured hierarchy are not hard-blocked but are flagged visually and in validation responses so the user consciously decides to proceed.
...
Each element appears **exactly once** within a strategy — either as a root node (no parent) or as a child of one and only one parent. The same element can be linked to different strategies, appearing under different parents in each.
...
Key observations:
- Each element appears **once per strategy** with one parent or as root
- **Goal Y** is the same record used in S1 under Perspective A AND in S2 under Perspective X
...
**Input modes:**
- **Percentage**: Enter 0–100%. Stored as decimal (e.g., 60% → 0.6000)
- **Impact Score**: Enter 1–10. Normalized at calculation time: `weight = score / SUM(all active sibling scores under same parent)`

**When no weight is set:** equal distribution among all included children (1/n).

### Shared Elements (Cross-Strategy Reuse)
When the same element is used in multiple strategies, it is marked in the element library and on its detail page with a **"used in X strategies" indicator** listing those strategies. Within a single strategy, each element appears exactly once — there is no within-strategy duplication. The element's global data (name, description, status) is the same in all strategies; only the edge context (parent, weight) differs per strategy.
...
| Status | Meaning |
|--------|---------|
| **DRAFT** | Strategy is being configured. Type levels and build config can be set. Elements can be added and linked. Progress and payments cannot be recorded. |
...
**BR-07** Within a single strategy, each element appears exactly once: either as a root node or as the child of exactly one parent. An element cannot appear under multiple parents in the same strategy.

**BR-08** The same element can be referenced in multiple strategies. In each strategy it has its own parent context (or is a root), its own edge weight, and its own display order. The element's global data remains identical across all strategies.
...
**BR-30** Since each element appears at most once per strategy (one parent or root), double-counting in budget aggregation within a single strategy is structurally impossible.

**BR-31** An element's consumed budget equals the sum of all its payment records globally across all strategies.

### Progress Rules

**BR-32** Progress can only be logged on elements whose type has the Trackable capability and whose status is ACTIVE.

**BR-33** Every Trackable element maintains two progress values:
...
**BR-34** For leaf elements (no active children in the strategy), `calculated_progress` is not applicable. Only `own_progress` is shown.

**BR-35** For parent elements (with active children), both values exist. `calculated_progress` is used for upward propagation. If the two values differ by more than a configurable threshold (default 5%), a discrepancy indicator is shown to alert the owner.

**BR-36** DRAFT and DEACTIVATED and DEPRECATED children are excluded from parent `calculated_progress`. Their edge weight is not counted in the denominator.

**BR-37** ON_HOLD and COMPLETED children are included using their frozen last value.

**BR-38** Edge weight defaults to equal distribution among all included children if not set.
...
Before persisting a new edge, the service performs a BFS from the proposed child upward through existing parent edges. If the proposed parent is reachable, the request is rejected with `GRAPH_CYCLE_DETECTED`.

**VR-03 Single Parent per Element per Strategy**
An element can be the child in at most one edge within a given strategy. Attempting to create a second parent edge for an element that already has a parent in the same strategy is rejected with `ELEMENT_ALREADY_HAS_PARENT_IN_STRATEGY`. The unique constraint on `(strategy_id, child_element_id)` enforces this at the database level.
...
**VR-09** Weight is optional. A weight of 0 is allowed (element participates structurally but contributes nothing to parent metrics — useful for informational links).
...
**VR-33** Each element type may appear only once in a strategy's level configuration.

**VR-34** Level order values must be unique per strategy.

**VR-35** Default weight input type must be one of: PERCENT, IMPACT.
...
Progress is calculated recursively, traversing bottom-up through the strategy graph. Status determines inclusion. Two values are maintained for Trackable elements: `own_progress` (manual) and `calculated_progress` (derived).

```
FUNCTION calculateProgress(elementId, strategyId):

  children = QUERY:
    SELECT l.child_element_id, l.weight, l.impact_display, l.weight_input_type,
           e.workflow_status
    FROM spm_element_link l
    JOIN spm_element e ON e.id = l.child_element_id
    WHERE l.parent_element_id = elementId
      AND l.strategy_id = strategyId
      AND e.deleted_at IS NULL
      AND l.deleted_at IS NULL

  -- Exclude DRAFT, DEACTIVATED, DEPRECATED children from calculation
  activeChildren = children WHERE status IN ('ACTIVE', 'ON_HOLD', 'COMPLETED')

  IF activeChildren IS EMPTY:
    -- Leaf node or all children excluded: return own_progress from progress log
    latestEntry = most recent spm_element_progress record for elementId
    RETURN latestEntry.rate IF exists ELSE 0

  -- Normalize weights
  FOR EACH child IN activeChildren:
    IF child.weight_input_type = 'IMPACT':
      totalSiblingImpact = SUM(c.impact_display FOR c IN activeChildren)
      child.effectiveWeight = child.impact_display / totalSiblingImpact
    ELSE IF child.weight_input_type = 'PERCENT':
      child.effectiveWeight = child.weight  -- already 0.0–1.0
    ELSE:  -- EQUAL
      child.effectiveWeight = 1.0 / COUNT(activeChildren)

  totalWeight = SUM(child.effectiveWeight FOR child IN activeChildren)

  IF totalWeight = 0:
    RETURN AVG(calculateProgress(child.id, strategyId) FOR child IN activeChildren)

  weightedSum = SUM(
    calculateProgress(child.id, strategyId) × child.effectiveWeight
    FOR child IN activeChildren
  )

  RETURN weightedSum / totalWeight  -- this is the element's calculated_progress
```
...
FUNCTION aggregateBudget(elementId, strategyId):
  ...
  -- DFS: collect all descendants in this strategy, excluding DEACTIVATED/DEPRECATED
  allDescendantIds = DFS_COLLECT(
    startId     = elementId,
    strategyId  = strategyId,
    excludeStatuses = ['DEACTIVATED', 'DEPRECATED']
  )

  allIds = allDescendantIds ∪ {elementId}

  -- Only sum elements whose type has is_budgetable = true
  budgetableIds = [id FOR id IN allIds WHERE element(id).type.is_budgetable = true]

  plannedTotal   = SUM(b.planned_budget   FROM spm_element_budget WHERE element_id IN budgetableIds)
  allocatedTotal = SUM(b.allocated_budget FROM spm_element_budget WHERE element_id IN budgetableIds)
  consumedTotal  = SUM(p.amount FROM spm_element_payment WHERE element_id IN budgetableIds AND deleted_at IS NULL)
...
FUNCTION calculateHealthScore(elementId, strategyId):
  ...
  indicatorLinks = QUERY:
    SELECT iel.indicator_id, iel.weight, iel.impact_display, iel.weight_input_type,
           i.workflow_status
...
**Rule**: If a user holds `element_editor` (or `element_owner`) permission on Element P, and Element P has a child Element C in Strategy S, then the user **inherits** `element_editor` on Element C within that strategy context, and recursively on all descendants of C.

This means: assigning a user as editor of a Goal automatically grants them editor access to all Initiatives (children) under that Goal in the strategy, without requiring explicit per-element assignments.
...
| **Edit element global fields** | element_editor, element_owner — OR — inherited from parent element (top-down) |
...
| **Link indicator to element** | element_editor or element_owner on the element, AND strategy_editor or strategy_owner on the strategy |
| **Link risk to element** | element_editor or element_owner on the element, AND strategy_editor or strategy_owner on the strategy |
...
type spm_element {
  ...
  permissions {
    ...
    can_edit   = editor or owner
                 or parent.can_edit         // top-down inheritance
    can_delete = owner
...
CREATE TABLE strategy_config (
    ...
    CONSTRAINT chk_weight_input CHECK (default_weight_input_type IN ('PERCENT','IMPACT'))
);
```
...
CREATE TABLE spm_element_link (
    ...
    deleted_at          TIMESTAMP WITHOUT TIME ZONE,
    deleted_by          VARCHAR(255),
    -- Each element can have at most ONE parent per strategy
    CONSTRAINT uq_child_per_strategy UNIQUE (strategy_id, child_element_id),
    CONSTRAINT chk_no_self_link      CHECK  (parent_element_id <> child_element_id),
    CONSTRAINT chk_impact_range      CHECK  (impact_display IS NULL OR (impact_display >= 1 AND impact_display <= 10)),
    CONSTRAINT chk_weight_input_type CHECK  (weight_input_type IN ('PERCENT','IMPACT','EQUAL'))
);
...
CREATE TABLE indicator_element_link (
    ...
    CONSTRAINT chk_ind_impact CHECK (impact_display IS NULL OR (impact_display >= 1 AND impact_display <= 10)),
    CONSTRAINT chk_ind_weight_type CHECK (weight_input_type IN ('PERCENT','IMPACT','EQUAL'))
);
```
...
| `initiative_impact_on_goal` | `spm_element_link` | impact_score → impact_display |
...
POST   /strategies/{id}/links                         Create edge { parentElementId, childElementId, weight?, impactDisplay?, weightInputType?, displayOrder? }
PUT    /strategies/{id}/links/{linkId}                Update weight or display order
DELETE /strategies/{id}/links/{linkId}                Remove edge

-- Graph queries
GET    /strategies/{id}/elements/{eid}/children       Direct children
GET    /strategies/{id}/elements/{eid}/parents        Direct parent (one or none)
...
POST   /strategies/{id}/indicator-links               { indicatorId, elementId, weight?, impactDisplay?, weightInputType? }
...
| `WeightInput` | Toggle between Percent and Impact Score input. Shows effective normalized weight. Defaults to strategy's configured weight input type. |
...
1. Create default element types for each organization (PERSPECTIVE, GOAL, INITIATIVE) with capabilities matching current model: Goal = Budgetable + Trackable + VisionAligned; Initiative = Budgetable + Trackable; Perspective = VisionAligned only
...
10. Migrate `initiative_impact_on_goal` → `spm_element_link` (impact_score → impact_display, weight_input_type = IMPACT)
...
16. Compute and store initial `spm_element_calculated_progress` for all active elements
17. Verify row counts match between old and new tables before proceeding to Phase 3
